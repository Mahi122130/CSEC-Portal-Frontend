...
export default function MultiStepForm() {
  ...
  const Toast = ({
    ...
    const borderColor =
      variant === "destructive" ? "border-red-300" : "border-green-300";

    return (
      <div
        className={`fixed top-4 right-4 z-50 p-4 rounded-md border ${bgColor} ${borderColor} ${textColor} shadow-lg max-w-xs`}
      >
      ...
      try {
        ...
        const initialData = {
          ...
          birth_date: personal_info?.birth_date
            ? new Date(personal_info.birth_date).toISOString().split("T")[0]
            : "",
          ...
          optional_birth_date: personal_info?.birth_date
            ? new Date(personal_info.birth_date).toISOString().split("T")[0]
            : "",
          joining_date: "",
          bio: "",
